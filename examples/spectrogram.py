...
fig, subplots = plt.subplots(SENSORS, 2)
fig.tight_layout()
...
# Plot initial lines which can be updated later
for i in range(0,SENSORS):
	ch_line,  = subplots[i,0].plot(range(QUEUE_SIZE),[0]*(QUEUE_SIZE), color=colors[i])
...
def animate(i):
	...
	if (emg_queue.full()):
		for i in range(0,SENSORS):
			...
			subplots[i,0].set_ylim(0,max(1024,max(channel)))
